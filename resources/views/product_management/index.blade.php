...
@if($store_type_id == $product_management['store_type_id'] && $genre_id == $product_management['genre_id'])
  <form method="post" action="{{ route('product_management.destroy', ['store_type_id' => $store_type_id, 'genre_id' => $genre_id, 'product_name' => $product_management['name']]) }}" class="mt-4 ml-4 mb-5">
    <input class="h2 mr-2" type="checkbox" name="product_name">{{ $product_management['name'] }}
...
<div class="container">
  <div class="row justify-content-md-center mt-5">
    ...
    <div class="text-center">
      ...
      <a class="btn btn-secondary col-5 mt-1" href="{{ route('product_management.edit', ['store_type_id' => $store_type_id, 'genre_id' => $genre_id]) }}">編集</a><br>
</form>
    <form action="{{ route('product_management.destroy', ['store_type_id' => $store_type_id, 'genre_id' => $genre_id]) }}"   method="POST" onsubmit="if(confirm('本当に削除しますか？')) { return true } else {return false };">
      @csrf
